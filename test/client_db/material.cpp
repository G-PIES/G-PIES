...
bool MaterialDescriptor::create_entity(ClientDb &db, Material &entity,
                                       int *sqlite_result_code) {
  return db.create_material(entity, sqlite_result_code);
}

bool MaterialDescriptor::read_entity(ClientDb &db, const int sqlite_id,
                                     Material &entity,
                                     int *sqlite_result_code) {
  return db.read_material(sqlite_id, entity, sqlite_result_code);
}

bool MaterialDescriptor::read_entities(ClientDb &db,
                                       std::vector<Material> &entities,
                                       int *sqlite_result_code) {
  return db.read_materials(entities, sqlite_result_code);
}

bool MaterialDescriptor::update_entity(ClientDb &db, const Material &entity,
                                       int *sqlite_result_code) {
  return db.update_material(entity, sqlite_result_code);
}

bool MaterialDescriptor::delete_entity(ClientDb &db, const Material &entity,
                                       int *sqlite_result_code) {
  return db.delete_material(entity, sqlite_result_code);
}

void MaterialDescriptor::randomize(Material &entity) {
  randomizer.material_randomize(entity);
}

void MaterialDescriptor::assert_equal(Material &first, Material &second,
