    ...
    )
{
    if (index > 1)
    {
        ...
        interstitials_out[index] += mtl_kernel.i_concentration_derivative(index) * mtl_kernel.delta_time;
        vacancies_out[index] += mtl_kernel.v_concentration_derivative(index) * mtl_kernel.delta_time;
    }
}