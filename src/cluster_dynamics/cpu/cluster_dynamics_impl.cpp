...
{
    return 2. * M_PI * cluster_radius(n) *
           i_bias_factor(n) *
           i_diffusion_val;
}
...
{
    ClusterDynamicsImpl* cd = reinterpret_cast<ClusterDynamicsImpl*>(user_data);

    cd->step_init();

    double *state_contents = N_VGetArrayPointer(state);
    double *state_derivative_contents = N_VGetArrayPointer(state_derivatives);

    cd->interstitials = state_contents;
    cd->vacancies = state_contents + cd->concentration_boundary + 2;
    cd->dislocation_density = state_contents + cd->state_size - 1;

    state_derivative_contents[1] = cd->i1_concentration_derivative() / cd->material.atomic_volume;

    for (int i = 2; i < cd->concentration_boundary + 1; ++i)
    {
        state_derivative_contents[i] = cd->i_concentration_derivative(i) / cd->material.atomic_volume;
    }
    state_derivative_contents[cd->concentration_boundary + 2 + 1] = cd->v1_concentration_derivative() / cd->material.atomic_volume;
    for (int i = 2; i < cd->concentration_boundary + 1; ++i)
    {
        state_derivative_contents[cd->concentration_boundary + 2 + i] = cd->v_concentration_derivative(i) / cd->material.atomic_volume;
    }
    state_derivative_contents[2 * (cd->concentration_boundary + 1) + 2] = cd->dislocation_density_derivative() / cd->material.atomic_volume;


    fprintf(stderr, "\n%g", cd->i_concentration_derivative(6) / cd->material.atomic_volume);
...
{
    int retval;
    state_size = 2 * (concentration_boundary + 1) + 4;
    ...
    /* Initial conditions */
    y = N_VNew_Serial(state_size, sunctx);
    if (check_retval((void *)y, "N_VNew_Serial", 0))
        fprintf(stderr, "Failed to create CVODES initial vector.");

    /* Initialize Values */

    std::memset(N_VGetArrayPointer(y), 0, state_size);
    dislocation_density = N_VGetArrayPointer(y) + state_size - 1;
    *dislocation_density = material.dislocation_density_0;
    interstitials = N_VGetArrayPointer(y);
    vacancies = N_VGetArrayPointer(y) + concentration_boundary + 2;
    ...
    retval = CVodeSStolerances(cvode_mem, 1e-3, 1e-3);
    if (check_retval(&retval, "CVodeSVtolerances", 1))
        fprintf(stderr, "Failed to set CVODES tolerances.");

    A = SUNDenseMatrix(state_size, state_size, sunctx);

    /* Create dense SUNLinearSolver object for use by CVode */
    LS = SUNLinSol_Dense(y, A, sunctx);
    if (check_retval((void *)LS, "SUNLinSol_Dense", 0))
        fprintf(stderr, "Failed to create CVODES linear solver.");

    CVodeSetUserData(cvode_mem, reinterpret_cast<void*>(this));
...
ClusterDynamicsState ClusterDynamicsImpl::run(double delta_time, double total_time)
{
    ...
    double out_time;
    const int retval = CVode(cvode_mem, time + total_time, y, &out_time, CV_NORMAL);
