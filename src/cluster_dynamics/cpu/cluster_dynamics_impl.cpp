#include <stdio.h>
#include <cstring>

#include "cluster_dynamics_impl.hpp"

// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 1a-1e
    The rate of production of interstital defects from the irradiation cascade for size (n) clusters.
*/
double ClusterDynamicsImpl::i_defect_production(size_t n)
{
    switch (n)
    {
        case 1: return reactor.recombination * reactor.flux *
                       (1. - reactor.i_bi - reactor.i_tri - reactor.i_quad);
        case 2: return reactor.recombination * reactor.flux * reactor.i_bi;
        case 3: return reactor.recombination * reactor.flux * reactor.i_tri;
        case 4: return reactor.recombination * reactor.flux * reactor.i_quad;

        default: break;
    }

    // cluster sizes > greater than 4 always zero
    return 0.;
};

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 1a-1e
    The rate of production of vacancy defects from the irradiation cascade for size (n) clusters.
*/
double ClusterDynamicsImpl::v_defect_production(size_t n)
{
    switch (n)
    {
        case 1: return reactor.recombination * reactor.flux *
                       (1. - reactor.v_bi - reactor.v_tri - reactor.v_quad);
        case 2: return reactor.recombination * reactor.flux * reactor.v_bi;
        case 3: return reactor.recombination * reactor.flux * reactor.v_tri;
        case 4: return reactor.recombination * reactor.flux * reactor.v_quad;

        default: break;
    }

    // cluster sizes > greater than 4 always zero
    return 0.;
};
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 2a
    The number of clusters that contain (in) interstitials per unit volume.

                  (1)     (2)                    (3)              (4)
    dCi(n) / dt = Gi(n) + a[i,n+1] * Ci(n + 1) - b[i,n] * Ci(n) + c[i,n-1] * Ci(n-1)
*/
double ClusterDynamicsImpl::i_clusters_delta(size_t in, size_t x, double delta_x)
{
    return
        // Spatial Dependency
        i_spatial_diffusion_loss_diffusion(delta_x, in, x)
        // (1)
        + i_defect_production(in)
        // (2)
        + iemission_vabsorption_np1(in + 1, x) * interstitials[in + 1][x]
        // // (3)
        - iemission_vabsorption_n(in, x) * interstitials[in][x]
        // // (4)
        + iemission_vabsorption_nm1(in - 1, x) * interstitials[in - 1][x];
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 2a
    The number of clusters that contain (vn) vacancies per unit volume.

                  (1)     (2)                    (3)              (4)
    dCv(n) / dt = Gv(n) + a[v,n+1] * Cv(n + 1) - b[v,n] * Cv(n) + c[v,n-1] * Cv(n-1)
*/
double ClusterDynamicsImpl::v_clusters_delta(size_t vn, size_t x, double delta_x)
{
    return
        // Spatial Dependency
        v_spatial_diffusion_loss_diffusion(delta_x, vn, x)
        // (1)
        + v_defect_production(vn)
        // (2)
        + vemission_iabsorption_np1(vn + 1, x) * vacancies[vn + 1][x]
        // (3)
        - vemission_iabsorption_n(vn, x) * vacancies[vn][x]
        // (4)
        + vemission_iabsorption_nm1(vn - 1, x) * vacancies[vn - 1][x];
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 2b
    The combined rate of emission of an interstitial and absorption of a vacancy by an interstitial loop of size (np1),
    both events leading to an interstitial loop of size n.

               (1)             (2)     (3)
    a[i,n+1] = B[i,v](n + 1) * Cv(1) + E[i,i](n + 1)
*/
double ClusterDynamicsImpl::iemission_vabsorption_np1(size_t np1, size_t x)
{
    return
        // (1)
        iv_absorption(np1) *
        // (2)
        vacancies[1][x] +
        // (3)
        ii_emission(np1);
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 2b
    The combined rate of emission of an interstitial and absorption of a vacancy by an interstitial loop of size (np1),
    both events leading to an interstitial loop of size n.

               (1)             (2)     (3)
    a[v,n+1] = B[v,i](n + 1) * Ci(1) + E[v,v](n + 1)
*/
double ClusterDynamicsImpl::vemission_iabsorption_np1(size_t np1, size_t x)
{
    return
        // (1)
        vi_absorption(np1) *
        // (2)
        interstitials[1][x] +
        // (3)
        vv_emission(np1);
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 2c
    The rate that a loop of size n can evolve toward a loop of size
    n + 1 absorbing an interstitial, or toward a loop of size
    n - 1 absorbing a vacancy or emitting an interstitial.

             (1)                 (2)                 (3)
    b[i,n] = B[i,v](n) * Cv(1) + B[i,i](n) * Ci(1) + E[i,i](n)
*/
double ClusterDynamicsImpl::iemission_vabsorption_n(size_t n, size_t x)
{
    return
        // (1)
        iv_absorption(n) * vacancies[1][x]
        // (2)
        + ii_absorption(n) * interstitials[1][x] * (1 - dislocation_promotion_probability(n, x))
        // (3)
        + ii_emission(n);
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 2c
    The rate that a loop of size n can evolve toward a loop of size
    n + 1 absorbing an vacancy, or toward a loop of size
    n - 1 absorbing an interstitial or emitting a vacancy.

             (1)                 (2)                 (3)
    b[v,n] = B[v,i](n) * Ci(1) + B[v,v](n) * Cv(1) + E[v,v](n)
*/
double ClusterDynamicsImpl::vemission_iabsorption_n(size_t n, size_t x)
{
    return
        // (1)
        vi_absorption(n) * interstitials[1][x]
        // (2)
        + vv_absorption(n) * vacancies[1][x]
        // (3)
        + vv_emission(n);
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 2d
    The rate that an interstitial loop of size n - 1 can evolve into a
    loop of size n by absorbing an interstitial.

               (1)           (2)
    c[i,n-1] = B[i,i](n-1) * Ci(1)
*/
double ClusterDynamicsImpl::iemission_vabsorption_nm1(size_t nm1, size_t x)
{
    return
        // (1)
        ii_absorption(nm1) *
        // (2)
        interstitials[1][x];
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 2d
    The rate that a vacancy loop of size n - 1 can evolve into a
    loop of size n by absorbing a vacancy.

               (1)           (2)
    c[v,n-1] = B[v,v](n-1) * Cv(1)
*/
double ClusterDynamicsImpl::vemission_iabsorption_nm1(size_t nm1, size_t x)
{
    return
        // (1)
        vv_absorption(nm1) *
        // (2)
        vacancies[1][x];
}
// --------------------------------------------------------------------------------------------

// Under construction: New Spatial Dependency Functions

double ClusterDynamicsImpl::i_spatial_diffusion_loss_diffusion(double delta_x, size_t n, size_t x)
{
   double fd1 = (interstitials[n][x] - interstitials[n][x-1]) / (delta_x);
   double fd2 = (interstitials[n][x+1] - interstitials[n][x]) / (delta_x);
   double sd = (fd2 - fd1) / (delta_x);

    return i_diffusion() * sd;
}

double ClusterDynamicsImpl::v_spatial_diffusion_loss_diffusion(double delta_x, size_t n, size_t x)
{
   double fd1 = (vacancies[n][x] - vacancies[n][x-1]) / (delta_x);
   double fd2 = (vacancies[n][x+1] - vacancies[n][x]) / (delta_x);
   double sd = (fd2 - fd1) / (delta_x);

    return v_diffusion() * sd;
}

// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3a
    Point defects concentrations per unit volume given a cluster size (in).
    ** in is only used in characteristic time calculations which rely on cluster sizes up to (in).

                (1)     (2)
    dCi(1)/dt = Gi(1) - Riv * Ci(1) * Cv(1) -
            (3)
            Ci(1) / (tAdi) -
            (4)
            Ci(1) / (tAgb) -
            (5)
            Ci(1) / (tAi) +
            (6)
            1 / (tEi)
*/
double ClusterDynamicsImpl::i1_cluster_delta(size_t x)
{
    return
        // (1)
        i_defect_production(1)
        // (2)
        - annihilation_rate() * interstitials[1][x] * vacancies[1][x]
        // (3)
        - interstitials[1][x] * i_dislocation_annihilation_time(x)
        // (4)
        - interstitials[1][x] * i_grain_boundary_annihilation_time(x)
        // (5)
        - interstitials[1][x] * i_absorption_time(x)
        // (6)
        + i_emission_time(x);
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3a
    Point defects concentrations per unit volume given a cluster size (vn).
    ** vn is only used in characteristic time calculations which rely on cluster sizes up to (vn).

            (1)     (2)
    Cv(1) = Gv(1) - Riv * Ci(1) * Cv(1) -
            (3)
            Cv(1) / (tAdv) -
            (4)
            Cv(1) / (tAgb) -
            (5)
            Cv(1) / (tAv) +
            (6)
            1 / (tEv)
*/
double ClusterDynamicsImpl::v1_cluster_delta(size_t x)
{
    return
        // (1)
        v_defect_production(1)
        // (2)
        - annihilation_rate() * interstitials[1][x] * vacancies[1][x]
        // (3)
        - vacancies[1][x] * v_dislocation_annihilation_time(x)
        // (4)
        - vacancies[1][x] * v_grain_boundary_annihilation_time(x)
        // (5)
        - vacancies[1][x] * v_absorption_time(x)
        // (6)
        + v_emission_time(x);
}
// --------------------------------------------------------------------------------------------

// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3b
    Characteristic time for emitting an interstitial by the population of interstital or vacancy
    clusters of size up to (nmax).

                (1)                      (2)                     (3)
    tEi(n) = SUM ( E[i,i](n) * Ci(n) ) + 4 * E[i,i](2) * Ci(2) + B[i,v](2) * Cv(2) * Ci(2)
*/
double ClusterDynamicsImpl::i_emission_time(size_t x)
{
   double time = 0.;
   for (size_t in = 3; in < concentration_boundary - 1; ++in)
   {
         time +=
         // (1)
         ii_emission(in) * interstitials[in][x];
   }

   time +=
      // (2)
      4. * ii_emission(2) * interstitials[2][x]
      // (3)
      + iv_absorption(2) * vacancies[2][x] * interstitials[2][x];

  return time;
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3b
    Characteristic time for emitting a vacancy by the population of interstital or vacancy
    clusters of size up to (nmax).

                (1)                           (2)                     (3)
    tEv(n) = SUM[n>0] ( E[v,v](n) * Cv(n) ) + 4 * E[v,v](2) * Cv(2) + B[v,i](2) * Cv(2) * Ci(2)
*/
double ClusterDynamicsImpl::v_emission_time(size_t x)
{
   double time = 0.;
   for (size_t vn = 3; vn < concentration_boundary - 1; ++vn)
   {
      time +=
         // (1)
         vv_emission(vn) * vacancies[vn][x];
   }

   time +=
      // (2)
      4. * vv_emission(2) * vacancies[2][x]
      // (3)
      + vi_absorption(2) * vacancies[2][x] * interstitials[2][x];

   return time;
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3c
    Characteristic time for absorbing an interstitial by the population of interstital or vacancy
    clusters of size up to (nmax).

                        (1)                              (2)
    tAi(n) = SUM[n>0] ( B[i,i](n) * Ci(n) ) + SUM[n>1] ( B[v,i](n) * Cv(n) )
*/
double ClusterDynamicsImpl::i_absorption_time(size_t x)
{
   double time = ii_absorption(1) * interstitials[1][x];
   for (size_t in = 2; in < concentration_boundary - 1; ++in)
   {
      time +=
         // (1)
         ii_absorption(in) * interstitials[in][x]
         // (2)
         + vi_absorption(in) * vacancies[in][x];
   }

   return time;
}


/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3c
    Characteristic time for absorbing a vacancy by the population of interstital or vacancy
    clusters of size up to (nmax).

                        (1)                              (2)
    tAv(n) = SUM[n>0] ( B[v,v](n) * Cv(n) ) + SUM[n>1] ( B[i,v](n) * Ci(n) )
*/
double ClusterDynamicsImpl::v_absorption_time(size_t x)
{
   double time = vv_absorption(1) * vacancies[1][x];
   for (size_t vn = 2; vn < concentration_boundary - 1; ++vn)
   {
      time +=
         // (1)
         vv_absorption(vn) * vacancies[vn][x]
         // (2)
         + iv_absorption(vn) * interstitials[vn][x];
   }

   return time;
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3d
    Annihilation rate of vacancies and insterstitals.

          (1)      (2)         (3)
    Riv = 4 * PI * (Di + Dv) * riv
*/
double ClusterDynamicsImpl::annihilation_rate()
{
    return
        // (1)
        4. * M_PI *
        // (2)
        (i_diffusion() + v_diffusion()) *
        // (3)
        material.recombination_radius;
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3e
    Characteristic time for annihilation of interstitials on dislocations.

           (1)   (2)    (3)
    tAdi = p  *  Di  *  Zi
*/
double ClusterDynamicsImpl::i_dislocation_annihilation_time(size_t x)
{
    return
        // (1)
        dislocation_density[x] *
        // (2)
        i_diffusion() *
        // (3)
        material.i_dislocation_bias;
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3e
    Characteristic time for annihilation of vacancies on dislocations.

           (1)   (2)    (3)
    tAdv = p  *  Dv  *  Zv
*/
double ClusterDynamicsImpl::v_dislocation_annihilation_time(size_t x)
{
    return
        // (1)
        dislocation_density[x] *
        // (2)
        v_diffusion() *
        // (3)
        material.v_dislocation_bias;
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3f
    Characteristic time for annihilation of interstitials on grain boundaries.

           (1)            (2)      (3)                            (4)                              (5)
    tAdi = 6 * Di * sqrt( p * Zi + SUM[n] ( B[i,i](n) * Ci(n) ) + SUM[n] ( B[v,i](n) * Cv(n) ) ) / d
*/
double ClusterDynamicsImpl::i_grain_boundary_annihilation_time(size_t x)
{
    return
        // (1)
        6. * i_diffusion() *
        sqrt
        (
            // (2)
            dislocation_density[x] *
            material.i_dislocation_bias
            // (3)
            + ii_sum_absorption_val[x]
            // (4)
            + vi_sum_absorption_val[x]
        ) /
        // (5)
        material.grain_size;
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 3f
    Characteristic time for annihilation of interstitials on grain boundaries.

           (1)            (2)      (3)                            (4)                              (5)
    tAdv = 6 * Di * sqrt( p * Zv + SUM[n] ( B[v,v](n) * Cv(n) ) + SUM[n] ( B[i,v](n) * Ci(n) ) ) / d
*/
double ClusterDynamicsImpl::v_grain_boundary_annihilation_time(size_t x)
{
    return
        // (1)
        6. * v_diffusion() *
        sqrt
        (
            // (2)
            dislocation_density[x] *
            material.v_dislocation_bias
            // (3)
            + vv_sum_absorption_val[x]
            // (4)
            + iv_sum_absorption_val[x]
        ) /
        // (5)
        material.grain_size;
}

// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 4a
    Rate of emission of an interstitial by an interstital loop of size (n).
*/
double ClusterDynamicsImpl::ii_emission(size_t n)
{
    return
        2. * M_PI * cluster_radius(n) *
        i_bias_factor(n) *
        i_diffusion() / material.atomic_volume *
        exp
        (
            -i_binding_energy(n) /
            (BOLTZMANN_EV_KELVIN * reactor.temperature)
        );
}


/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 4b
    Rate of absorption of an interstitial by an interstital loop of size (n).
*/
double ClusterDynamicsImpl::ii_absorption(size_t n)
{
    return
        2. * M_PI * cluster_radius(n) *
        i_bias_factor(n) *
        i_diffusion();
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 4c
    Rate of absorption of an interstitial by a vacancy loop of size (n).
*/
double ClusterDynamicsImpl::iv_absorption(size_t n)
{
    return
        2. * M_PI * cluster_radius(n) *
        v_bias_factor(n) *
        v_diffusion();
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 4d
    Rate of emission of a vacancy by a vacancy loop of size (n).
*/
double ClusterDynamicsImpl::vv_emission(size_t n)
{
    return
        2. * M_PI * cluster_radius(n) *
        v_bias_factor(n) *
        v_diffusion() *
        exp
        (
            -v_binding_energy(n) /
            (BOLTZMANN_EV_KELVIN * reactor.temperature)
        );
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 4e
    Rate of absorption of a vacancy by a vacancy loop of size (n).
*/
double ClusterDynamicsImpl::vv_absorption(size_t n)
{
    return
        2. * M_PI * cluster_radius(n) *
        v_bias_factor(n) *
        v_diffusion();
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 4f
    Rate of absorption of a vacancy by an interstitial loop of size (n).
*/
double ClusterDynamicsImpl::vi_absorption(size_t n)
{
    return
        2. * M_PI * cluster_radius(n) *
        i_bias_factor(n) *
        i_diffusion();
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 5
    Interstitial bias factor.
*/
double ClusterDynamicsImpl::i_bias_factor(size_t in)
{
    return
        material.i_dislocation_bias +
        (
            std::sqrt
            (
                    material.burgers_vector /
                    (8. * M_PI * material.lattice_param)
            ) *
            material.i_loop_bias -
            material.i_dislocation_bias
        ) *
        1. /
        std::pow
        (
            (double)in,
            material.i_dislocation_bias_param / 2.
        );
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 5
    Vacancy bias factor.
*/
double ClusterDynamicsImpl::v_bias_factor(size_t vn)
{
    return
        material.v_dislocation_bias +
        (
            std::sqrt
            (
                    material.burgers_vector /
                    (8. * M_PI * material.lattice_param)
            ) *
            material.v_loop_bias -
            material.v_dislocation_bias
        ) *
        1. /
        std::pow
        (
            (double)vn,
            material.v_dislocation_bias_param / 2.
        );
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 6
    Interstitial binding energy.
*/
double ClusterDynamicsImpl::i_binding_energy(size_t in)
{
    return
        material.i_formation
        + (material.i_binding - material.i_formation) / (std::pow(2., .8) - 1.) *
        (std::pow((double)in, .8) - std::pow((double)in - 1., .8));
}

/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 6
    Vacancy binding energy.
*/
double ClusterDynamicsImpl::v_binding_energy(size_t vn)
{
    return
        material.v_formation
        + (material.v_binding - material.v_formation) / (std::pow(2., .8) - 1) *
        (std::pow((double)vn, .8) - std::pow((double)vn - 1., .8));
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  G. Was / Fundamentals of Radiation Materials Science (2nd Edition) (2017), pg 193, 4.59
*/
double ClusterDynamicsImpl::i_diffusion()
{
    return material.i_diffusion_0 * std::exp(-material.i_migration / (BOLTZMANN_EV_KELVIN * reactor.temperature));
}

/*  G. Was / Fundamentals of Radiation Materials Science (2nd Edition) (2017), pg 193, 4.59
*/
double ClusterDynamicsImpl::v_diffusion()
{
   return material.v_diffusion_0 * std::exp(-material.v_migration / (BOLTZMANN_EV_KELVIN * reactor.temperature));
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  N. Sakaguchi / Acta Materialia 1131 (2001), 3.10
*/
double ClusterDynamicsImpl::mean_dislocation_cell_radius(size_t x)
{
   double r_0_factor = 0.;
   for (size_t i = 1; i < concentration_boundary; ++i)
   {
      r_0_factor += cluster_radius(i) * interstitials[i][x];
   }

   return 1 / std::sqrt((2. * M_PI * M_PI / material.atomic_volume) * r_0_factor + M_PI * dislocation_density[x]);
}

// --------------------------------------------------------------------------------------------
/*  N. Sakaguchi / Acta Materialia 1131 (2001), 3.12
*/
double ClusterDynamicsImpl::dislocation_promotion_probability(size_t n, size_t x)
{
   double dr = cluster_radius(n + 1) - cluster_radius(n);

   return (2. * cluster_radius(n) * dr + std::pow(dr, 2.))
      / (M_PI * mean_dislocation_radius_val[x] / 2. - std::pow(cluster_radius(n), 2.));
}
// --------------------------------------------------------------------------------------------


// --------------------------------------------------------------------------------------------
/*  C. Pokor / Journal of Nuclear Materials 326 (2004), 8
*/
double ClusterDynamicsImpl::dislocation_density_delta(size_t x)
{
   double gain = 0.0;
   for (size_t n = 1; n < concentration_boundary; ++n)
   {
      gain += cluster_radius(n) * dislocation_promotion_probability(n, x) * ii_absorption(n) * interstitials[n][x];
   }

   gain *= 2. * M_PI / material.atomic_volume;

   return
      gain
      - reactor.dislocation_density_evolution *
      std::pow(material.burgers_vector, 2.) *
      std::pow(dislocation_density[x], 3. / 2.);
}

// --------------------------------------------------------------------------------------------

// --------------------------------------------------------------------------------------------
/*  G. Was / Fundamentals of Radiation Materials Science (2nd Edition) (2017), pg. 346, 7.63
*/
double ClusterDynamicsImpl::cluster_radius(size_t n)
{
    return std::sqrt(std::sqrt(3.) * std::pow(material.lattice_param, 2.) * (double)n / (4. * M_PI));
}
// --------------------------------------------------------------------------------------------



// --------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------
/*
 *  SIMULATION CONTROL FUNCTIONS
 */
// --------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------

void ClusterDynamicsImpl::step_init(size_t x)
{
  mean_dislocation_radius_val[x] = mean_dislocation_cell_radius(x);
  ii_sum_absorption_val[x] = ii_sum_absorption(concentration_boundary - 1, x);
  iv_sum_absorption_val[x] = iv_sum_absorption(concentration_boundary - 1, x);
  vi_sum_absorption_val[x] = vi_sum_absorption(concentration_boundary - 1, x);
  vv_sum_absorption_val[x] = vv_sum_absorption(concentration_boundary - 1, x);
}

bool ClusterDynamicsImpl::step(double delta_time, double delta_x)
{
  #ifdef USE_CUDA
    cudaMemcpy(thrust::raw_pointer_cast(self), this, sizeof(ClusterDynamicsImpl), cudaMemcpyHostToDevice);
  #endif

  for (size_t x = 0; x <= max_spatial_dim; x++)
    step_init(x);

  bool state_is_valid = update_clusters_1(delta_time, delta_x);
  update_clusters(delta_time, delta_x);

  for (size_t x = 0; x <= max_spatial_dim; x++)
    dislocation_density[x] += dislocation_density_delta(x) * delta_time;

  interstitials = interstitials_temp;
  vacancies = vacancies_temp;

  return state_is_valid;
}

bool ClusterDynamicsImpl::update_clusters_1(double delta_time, double delta_x)
{
  bool state_is_valid = true;

  for (size_t x = 1; x <= max_spatial_dim; x++)
  {
    interstitials_temp[1][x] += i1_cluster_delta(x) * delta_time;
    vacancies_temp[1][x] += v1_cluster_delta(x) * delta_time;

    state_is_valid = state_is_valid && validate(1, x);
  }

  return state_is_valid;
}

bool ClusterDynamicsImpl::update_clusters(double delta_time, double delta_x)
{
   bool state_is_valid = true;

   for (size_t n = 2; n < concentration_boundary; ++n)
   {
      for (size_t x = 1; x <= max_spatial_dim; x++)
      {
        interstitials_temp[n][x] += i_clusters_delta(n, x, delta_x) * delta_time;
        vacancies_temp[n][x] += v_clusters_delta(n, x, delta_x) * delta_time;

        state_is_valid = state_is_valid && validate(n, x);
      }

   }

   return state_is_valid;
}

ClusterDynamicsImpl::~ClusterDynamicsImpl()
{
  #ifdef USE_CUDA
    thrust::device_free(self);
  #endif
}

double ClusterDynamicsImpl::ii_sum_absorption(size_t nmax, size_t x)
{
   double emission = 0.;
   for (size_t vn = 1; vn < nmax; ++vn)
   {
      emission += ii_absorption(vn) * interstitials[vn][x];
   }

   return emission;
}

double ClusterDynamicsImpl::iv_sum_absorption(size_t nmax, size_t x)
{
   double emission = 0.;
   for (size_t n = 1; n < nmax; ++n)
   {
      emission += iv_absorption(n) * interstitials[n][x];
   }

   return emission;
}

double ClusterDynamicsImpl::vv_sum_absorption(size_t nmax, size_t x)
{
   double emission = 0.;
   for (size_t n = 1; n < nmax; ++n)
   {
      emission += vv_absorption(n) * vacancies[n][x];
   }

   return emission;
}

double ClusterDynamicsImpl::vi_sum_absorption(size_t nmax, size_t x)
{
   double emission = 0.;
   for (size_t n = 1; n < nmax; ++n)
   {
      emission += vi_absorption(n) * vacancies[n][x];
   }

   return emission;
}

bool ClusterDynamicsImpl::validate(size_t n, size_t x)
{
  if (!(!std::isnan(interstitials_temp[n][x]) &&
        !std::isinf(interstitials_temp[n][x]) &&
        !std::isnan(vacancies_temp[n][x]) &&
        !std::isinf(vacancies_temp[n][x]) &&
        !(interstitials_temp[n][x] < 0) &&
        !(vacancies_temp[n][x] < 0))) {
    try {
      throw ClusterDynamicsException();
    }
    catch (ClusterDynamicsException c) {
      printf("Caught cluster dynamics exception at spatial dim: %d and cluster size: %d \n", x, n);
    }
  }
    return
        !std::isnan(interstitials_temp[n][x]) &&
        !std::isinf(interstitials_temp[n][x]) &&
        !std::isnan(vacancies_temp[n][x]) &&
        !std::isinf(vacancies_temp[n][x]) &&
        !(interstitials_temp[n][x] < 0) &&
        !(vacancies_temp[n][x] < 0);
}





// --------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------
/*
 *  PUBLIC INTERFACE FUNCTIONS
 */
// --------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------



// TODO - clean up the uses of random +1/+2/-1/etc throughout the code
ClusterDynamicsImpl::ClusterDynamicsImpl(size_t concentration_boundary, size_t max_spatial_dim, NuclearReactor reactor, Material material)
  : concentration_boundary(concentration_boundary), max_spatial_dim(max_spatial_dim), reactor(reactor), material(material),
    interstitials(concentration_boundary + 1, std::vector<double>(max_spatial_dim + 1, 0.0)),
    vacancies(concentration_boundary + 1, std::vector<double>(max_spatial_dim + 1, 0.0)),
    interstitials_temp(concentration_boundary + 1, std::vector<double>(max_spatial_dim + 1, 0.0)),
    vacancies_temp(concentration_boundary + 1, std::vector<double>(max_spatial_dim + 1, 0.0)),
    dislocation_density(max_spatial_dim + 1, material.dislocation_density_0), time(0.0), mean_dislocation_radius_val(max_spatial_dim + 1, 0.0),
    ii_sum_absorption_val(max_spatial_dim + 1, 0.0), iv_sum_absorption_val(max_spatial_dim + 1, 0.0), vv_sum_absorption_val(max_spatial_dim + 1, 0.0),
    vi_sum_absorption_val(max_spatial_dim + 1, 0.0)
    {
  #ifdef USE_CUDA
    ClusterDynamicsImpl* ptr;
    cudaMalloc(&ptr, sizeof(ClusterDynamicsImpl));
    self = thrust::device_ptr<ClusterDynamicsImpl>(ptr);
  #else
    self = this;
  #endif
}

ClusterDynamicsState ClusterDynamicsImpl::run(double delta_time, double total_time, double x_delta)
{
    bool state_is_valid = true;

    for (double endtime = time + total_time; time < endtime; time += delta_time)
    {
        state_is_valid = step(delta_time, x_delta);
        if (!state_is_valid) break;
    }

    // Need to fix the resizing here

    //vector<double> is(interstitials);
    //vector<double> vs(vacancies);

    return ClusterDynamicsState
    {
        .valid = state_is_valid,
        .time = time,
        .interstitials = interstitials,
        .vacancies = vacancies,
        //.interstitials = std::vector<std::vector<double>>(is.begin(), is.end() - 1),
        //.vacancies = std::vector<std::vector<double>>(vs.begin(), vs.end() - 1),
        .dislocation_density = dislocation_density
    };
}

Material ClusterDynamicsImpl::get_material()
{
    return material;
}

void ClusterDynamicsImpl::set_material(Material material)
{
    this->material = material;
}

NuclearReactor ClusterDynamicsImpl::get_reactor()
{
    return reactor;
}

void ClusterDynamicsImpl::set_reactor(NuclearReactor reactor)
{
    this->reactor = reactor;
}
