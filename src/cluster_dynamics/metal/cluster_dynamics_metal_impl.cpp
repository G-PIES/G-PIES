...
gp_float ClusterDynamicsImpl::v_bias_factor(size_t n) const {
    return material.v_dislocation_bias +
           (std::sqrt(material.burgers_vector /
            ...
            material.v_dislocation_bias) *
               1. /
               std::pow((gp_float)n, material.v_dislocation_bias_param / 2.);
}
 ...
 *    \dwn{P_{unf}(n) =}
 *    \frac
 *       {\ann{1}{2r_i(n)(r_i(n+1) \dwn{-} r_i(n))} \dwn{+}
 * \ann{2}{(r_i(n+1)-r_i)^2}}
...
void ClusterDynamicsImpl::step_init() {
    ...
    mtl_args.i_diffusion_val = i_diffusion_val;
    mtl_args.v_diffusion_val = v_diffusion_val;
    mtl_args.ii_sum_absorption_val = ii_sum_absorption_val;
    mtl_args.iv_sum_absorption_val = iv_sum_absorption_val;
    mtl_args.vi_sum_absorption_val = vi_sum_absorption_val;
    mtl_args.vv_sum_absorption_val = vv_sum_absorption_val;
    mtl_args.mean_dislocation_radius_val = mean_dislocation_radius_val;
}
...
bool ClusterDynamicsImpl::step(gp_float delta_time) {
    ...
    gp_float *interstitials_in =
        reinterpret_cast<gp_float *>(mtl_interstitials_in->contents());
    gp_float *vacancies_in =
        reinterpret_cast<gp_float *>(mtl_vacancies_in->contents());
    memcpy(interstitials_in + 1, interstitials.data() + 1,
           sizeof(gp_float) * concentration_boundary);
    memcpy(vacancies_in + 1, vacancies.data() + 1,
           sizeof(gp_float) * concentration_boundary);
    ...
    gp_float *interstitials_out =
        reinterpret_cast<gp_float *>(mtl_interstitials_out->contents());
    gp_float *vacancies_out =
        reinterpret_cast<gp_float *>(mtl_vacancies_out->contents());
    memcpy(interstitials.data() + 2, interstitials_out + 2,
           sizeof(gp_float) * (concentration_boundary - 1));
    memcpy(vacancies.data() + 2, vacancies_out + 2,
           sizeof(gp_float) * (concentration_boundary - 1));

    mtl_args.dislocation_density +=
        dislocation_density_derivative() * delta_time;
...
ClusterDynamicsImpl::~ClusterDynamicsImpl() { mtl_ar_pool->release(); }
...
MaterialImpl ClusterDynamicsImpl::get_material() { return material; }

void ClusterDynamicsImpl::set_material(const MaterialImpl &material) {
    this->material = material;
}

NuclearReactorImpl ClusterDynamicsImpl::get_reactor() { return reactor; }
...
void ClusterDynamicsImpl::mtl_encode_command(
    ...
    mtl_compute_encoder->setBytes(&mtl_args, sizeof(ClusterDynamicsMetalArgs),
                                  0);
