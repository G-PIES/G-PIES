...
gp_float sample_interval = time_delta;  // How often (in seconds) to record the state
...
void print_start_message() {
  ...
  std::cout << "\nG-PIES simulation started\n"
            ...
            << "  time delta: " << time_delta << "  simulation time: " << simulation_time
            << "  max cluster size: " << static_cast<int>(config.max_cluster_size)
            << "  data validation: " << (config.data_validation_on ? "on" : "off") << std::endl
            ...
            << "  max integration step: " << config.max_integration_step << std::endl;
}
...
void print_state(const ClusterDynamicsState& state) {
  ...
  for (size_t n = 1; n < size; ++n) {
    for (size_t n = 1; n < size; ++n) {
      os << (long long unsigned int)n << "\t\t\t\t\t" << std::setprecision(13) << state.interstitials[n] << "\t\t\t"
         << std::setprecision(15) << state.vacancies[n] << std::endl;
    }

    os << "\nDislocation Network Density: " << state.dislocation_density << std::endl;
  }
}
...
void print_simulation_history(ClientDb& db, bool print_details) {
  ...
  os << "\nSimulation History\tCount: " << static_cast<long long unsigned int>(simulations.size()) << std::endl;
  ...
  if (!simulations.empty()) {
    ...
    for (HistorySimulation s : simulations) {
      os << s.sqlite_id << " ~ " << static_cast<unsigned long long>(s.max_cluster_size) << " ~ " << s.simulation_time
         << " ~ " << s.time_delta << " ~ " << s.reactor.species << " ~ " << s.material.species << " ~ "
...
void update_for_sensitivity_analysis(ClusterDynamics& cd, NuclearReactor& reactor, Material& material,
...
void print_info_SA304() {
  Material material;
  materials::SA304(material);
  std::cout << material.species << "\ninterstitial migration: " << material.get_i_migration() << " eV"
            << "\nvacancy migration: " << material.get_v_migration() << " eV"
            << "\ninitial interstitial diffusion: " << material.get_i_diffusion_0() << " cm^2/s"
            << "\ninitial vacancy diffusion: " << material.get_v_diffusion_0() << " cm^2/s"
            << "\ninterstitial formation: " << material.get_i_formation() << " eV"
            ...
            << "\nrecombination radius: " << material.get_recombination_radius() << " cm"
            << "\ninterstitial loop bias: " << material.get_i_loop_bias()
            << "\ninterstitial dislocation bias: " << material.get_i_dislocation_bias()
            << "\ninterstitial dislocation bias param: " << material.get_i_dislocation_bias_param()
            << "\nvacancy loop bias: " << material.get_v_loop_bias()
            << "\nvacancy dislocation bias: " << material.get_v_dislocation_bias()
            << "\nvacancy dislocation bias param: " << material.get_v_dislocation_bias_param()
            << "\ninitial dislocation density: " << material.get_dislocation_density_0() << " cm^-2"
            << "\ngrain size: " << material.get_grain_size() << " cm"
            << "\nlattice parameter (FCC Nickel Approximation): " << material.get_lattice_param() << " cm"
            << "\nburgers vector (lattice_parameter / sqrt(2)): " << material.get_burgers_vector() << " cm"
            << "\natomic volume (lattice_parameter^3 / 4): " << material.get_atomic_volume() << "cm^3" << std::endl;
}
...
void print_info_OSIRIS() {
  ...
  std::cout << reactor.species << "\nflux: " << reactor.get_flux() << " dpa/s"
            ...
            << "\nbi-interstitial generation rate (CURRENTLY UNUSED): " << reactor.get_i_bi()
            << "\ntri-interstitial generation rate: " << reactor.get_i_tri()
            << "\nquad-interstitial generation rate: " << reactor.get_i_quad()
            << "\nbi-vacancy generation rate (CURRENTLY UNUSED): " << reactor.get_v_bi()
            << "\ntri-vacancy generation rate: " << reactor.get_v_tri()
            << "\nquad-vacancy generation rate: " << reactor.get_v_quad()
            << "\ndislocation density evolution: " << reactor.get_dislocation_density_evolution() << std::endl;
}
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    all_options.add_options()("help", "display help message")("material-info",
                                                              "display information on SA304 stainless steel")(
        "reactor-info", "display information on the OSIRIS reactor")("version", "display version information")(
        "csv", "csv output formatting")("step-print", "display simulation state at every time step")(
        "output-file", po::value<std::string>()->value_name("filename"), "write simulation output to a file")(
        "db", "database options")("data-validation",
                                  po::value<std::string>()->value_name("toggle")->implicit_value("on"),
                                  "turn on/off data validation (on by default)")(
        "max-cluster-size", po::value<size_t>()->implicit_value(config.max_cluster_size),
        "set the max size of defect clustering to model")("time",
                                                          po::value<gp_float>()->implicit_value(simulation_time),
        ...
        "sample-interval", po::value<gp_float>()->implicit_value(sample_interval),
        "how often to record simulation environment state (in seconds)")(
        "relative-tolerance", po::value<gp_float>()->implicit_value(config.relative_tolerance),
        "scalar relative tolerance for integration error")(
        "absolute-tolerance", po::value<gp_float>()->implicit_value(config.absolute_tolerance),
        "absolute relative tolerance for integration error")(
        "max-num-integration-steps", po::value<size_t>()->implicit_value(config.max_num_integration_steps),
        "maximum allowed number of integration steps to achieve a single estimation")(
        "min-integration-step", po::value<gp_float>()->implicit_value(config.min_integration_step),
        "minimum step size for integration")("max-integration-step",
    ...
    db_options.add_options()("history,h", "display simulation history")(
        "history-detail", "display detailed simulation history")("run,r", po::value<int>()->value_name("id"),
    ...
    sa_options.add_options()("sensitivity-analysis-help",
        ...
        "mode (REQUIRED)")("sensitivity-var,v", po::value<std::string>()->value_name("var name"),
    ...
    if (vm.count("max-cluster-size")) {
      ...
      if (mcs <= 0) throw GpiesException("Value for max-cluster-size must be a positive, non-zero integer.");
    ...
    if (vm.count("time")) {
      ...
      if (st <= 0.) throw GpiesException("Value for time must be a positive, non-zero decimal.");
    ...
    if (vm.count("data-validation")) {
      config.data_validation_on = 0 == vm["data-validation"].as<std::string>().compare("on");
    }
    ...
    if (vm.count("db")) {  // DATABASE
      if (vm.count("history")) {
        ...
        if (db.delete_simulations()) {
          std::cout << "Simulation History Cleared. " << db.changes() << " Simulation(s) Deleted.\n";
        }
      ...
      if (vm.count("num-sims") && vm.count("sensitivity-var") && vm.count("sensitivity-var-delta")) {
        sa_num_simulations = vm["num-sims"].as<int>();

        if (sa_num_simulations <= 0) throw GpiesException("Value for num-sims must be a positive, non-zero integer.");
      ...
      for (size_t n = 0; n < sa_num_simulations; n++) {
        ...
        ClusterDynamicsState state;
        update_for_sensitivity_analysis(cd, config.reactor, config.material, static_cast<gp_float>(n) * sa_var_delta);
      ...
      HistorySimulation history_simulation(config.max_cluster_size, simulation_time, time_delta, config.reactor,
